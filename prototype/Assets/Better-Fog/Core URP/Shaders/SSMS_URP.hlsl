...
float4 frag_prefilter(Varyings i) : SV_Target
{
    ...
    // SMSS 
    float depth = AdjustDepth(SAMPLE_TEXTURE2D(_FogFactor_RT,sampler_FogFactor_RT, i.uv)); 
...
#ifdef ANTI_FLICKER_ON
    return ToFloat4(DownsampleAntiFlickerFilter(i.uv));
#else
    return ToFloat4(DownsampleFilter(i.uv));
#endif
}
...
{
    UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(i);
    float3 base = ToFloat3(SAMPLE_TEXTURE2D(_BaseTex,sampler_BaseTex, i.uv));
    float3 blur = UpsampleFilter(i.uv);
...
{
    UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(i);
	float4 base = SAMPLE_TEXTURE2D(_BaseTex,sampler_BaseTex, i.uv);
    float3 blur = UpsampleFilter(i.uv);
