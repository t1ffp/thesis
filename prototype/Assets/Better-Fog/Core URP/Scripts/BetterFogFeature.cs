...
namespace INab.BetterFog.URP
{
	public class BetterFogFeature : ScriptableRendererFeature
	{
		...
		{
			private RenderTargetHandle m_FogFactorRT;
			private RenderTargetHandle m_CustomDepthRT;
			private RenderTargetHandle m_FogOffsetRT;
			...
			public BetterFogPass(BetterFogSettings settings, Material depthBlit, Material fogBlit, Material finalBlit, Material SSMS)
			{
				...
				m_CustomDepthRT.Init("_CustomDepth");
				m_FogFactorRT.Init("_FogFactor_RT");
				m_FogOffsetRT.Init("_FogOffset");
				m_TemporaryRT.Init("_TemporaryRT");
			}
			...
			public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
			{
				...
				cmd.GetTemporaryRT(m_TemporaryRT.id, textureDescriptor, FilterMode.Point);

				textureDescriptor.colorFormat = RenderTextureFormat.RGHalf;
				cmd.GetTemporaryRT(m_FogFactorRT.id, textureDescriptor, FilterMode.Point);

				if (m_CurrentSettings._UseCustomDepthTexture)
                {
					textureDescriptor.colorFormat = RenderTextureFormat.RFloat;
					cmd.GetTemporaryRT(m_CustomDepthRT.id, textureDescriptor, FilterMode.Point);
				}

				if (m_CurrentSettings._UseFogOffsetTexture)
				{
					textureDescriptor.colorFormat = RenderTextureFormat.RGHalf;
					cmd.GetTemporaryRT(m_FogOffsetRT.id, textureDescriptor, FilterMode.Point);
				}
			}
			...
			public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
			{
				...
				if (m_CurrentSettings._UseCustomDepthTexture)
				{
					...
					Blit(cmd, renderer.cameraColorTarget, m_CustomDepthRT.Identifier(), m_DepthBlit, 0);

					cmd.SetRenderTarget(m_CustomDepthRT.Identifier());
					...
					cmd.SetGlobalTexture("_CustomDepth", m_CustomDepthRT.Identifier());
				}
				...
				if (m_CurrentSettings._UseFogOffsetTexture)
				{
					...
					cmd.SetRenderTarget(m_FogOffsetRT.Identifier());
					...
					cmd.SetGlobalTexture("_FogOffset", m_FogOffsetRT.Identifier());
				}
				else
                {
				...
				if (!m_BetterFogVolume._UseSSMS.value)
				{
					...
					Blit(cmd, renderer.cameraColorTarget, m_FogFactorRT.Identifier(), m_FogBlit, 0);
					cmd.SetGlobalTexture("_FogFactor_RT", m_FogFactorRT.Identifier());

					// Camera fog blit
					Blit(cmd, renderer.cameraColorTarget, m_TemporaryRT.Identifier(), m_FinalBlit, 0);
					Blit(cmd, m_TemporaryRT.Identifier(), renderer.cameraColorTarget);
				}
				else
				{
					// Fog blit to FogFactorRT
					...
					cmd.GetTemporaryRT(prefilteredRT, cameraDesc.width,cameraDesc.height,0,rtFilterMode,rtFormat);

					var pass = 0;
					cmd.SetGlobalTexture("_MainTex", m_TemporaryRT.Identifier());
					Blit(cmd, m_TemporaryRT.Identifier(), prefilteredRT, m_SSMS, pass);
					...
					{
						cmd.GetTemporaryRT(_blurBuffer1[level], tw, th, 0, rtFilterMode, rtFormat);
						cmd.GetTemporaryRT(_blurBuffer2[level], tw, th, 0, rtFilterMode, rtFormat);
					...
					// finish process
					cmd.SetGlobalTexture("_BaseTex", m_TemporaryRT.Identifier());
					pass = 4;
			...
			{
				cmd.ReleaseTemporaryRT(m_FogFactorRT.id);
				cmd.ReleaseTemporaryRT(m_CustomDepthRT.id);
				cmd.ReleaseTemporaryRT(m_FogOffsetRT.id);
